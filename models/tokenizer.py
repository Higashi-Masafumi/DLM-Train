...
class Tokenizer:
    def __init__(self, checkpoint_dir: Path) -> None:
        ...
        if (vocabulary_path := checkpoint_dir / "tokenizer.model").is_file():
            ...
            # huggingfaceのpretrained modelを指定された場合
            from transformers import AutoTokenizer
            hf_tokenizer = AutoTokenizer.from_pretrained(str(checkpoint_dir))
            if hf_tokenizer.is_fast:
                self.processor = hf_tokenizer.backend_tokenizer
                self.backend = "huggingface"
                bos_token = hf_tokenizer.bos_token
                self.bos_id = (
                    self.token_to_id(bos_token) if bos_token is not None else None
                )
                self.eos_id = self.token_to_id(hf_tokenizer.eos_token)
            else:
                raise ValueError(
                    f"The tokenizer at {checkpoint_dir} is not a fast tokenizer. Please provide a fast tokenizer."
                )
