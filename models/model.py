...
class GPT(nn.Module):
    ...
    def _init_weights(self, module: nn.Module, n_layer) -> None:
        ...
        if isinstance(module, nn.Embedding):
            torch.nn.init.normal_(module.weight, mean=0.0, std=math.sqrt(2.0 / 5 / self.config.n_embed))
            # RWKV: set it to 1e-4
        ...
        for name, p in module.named_parameters():
            if (name == "proj.weight" and isinstance(module, LLaMAMLP)) or (name == "w3.weight" and isinstance(module, SwiGLU) or (name=="proj.weight" and isinstance(module, CausalSelfAttention))):  #if use xformer swiglu, fc2 layer will be renamed to w3
                nn.init.normal_(p, mean=0.0, std=1 / math.sqrt(self.config.n_embed)  /  n_layer)
    ...
    def forward(
        self, idx: torch.Tensor, max_seq_length: int | None = None, input_pos: torch.Tensor | None = None
    ) -> torch.Tensor:
        B, T = idx.size()
...
class Block(nn.Module):
    def __init__(self, config: Config) -> None:
        super().__init__()
        self.norm_1 = config.norm_class(config.n_embed, eps=config.norm_eps)
        self.attn = CausalSelfAttention(config)
        if not config.shared_attention_norm:
            self.norm_2 = config.norm_class(config.n_embed, eps=config.norm_eps)
...
class GptNeoxMLP(nn.Module):
    def __init__(self, config: Config) -> None:
        ...
        self.proj = nn.Linear(config.intermediate_size, config.n_embed, bias=config.bias)
...
class LLaMAMLP(nn.Module):
    def __init__(self, config: Config) -> None:
        super().__init__()
        assert config.intermediate_size is not None
        # self.fc_1 = nn.Linear(config.n_embed, config.intermediate_size, bias=config.bias)
        # self.fc_2 = nn.Linear(config.n_embed, config.intermediate_size, bias=config.bias)
        # self.proj = nn.Linear(config.intermediate_size, config.n_embed, bias=config.bias)
        self.swiglu = SwiGLU(config.n_embed,config.intermediate_size, bias=False, _pack_weights=False)
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x_fc_1 = self.fc_1(x)
        # x_fc_2 = self.fc_2(x)
