...
import math
from models.model import GPT, Config
...
def flops_per_param(config: Config, n_params: int) -> int:
    ...
    embed_dim = getattr(config, "n_embed", None)
    if embed_dim is None:
        embed_dim = getattr(config, "n_embd", None)
    if embed_dim is None:
        raise AttributeError("Config is missing both 'n_embed' and 'n_embd' attributes required for FLOP estimation")
    attn_flops_per_seq = config.n_layer * 2 * 2 * (embed_dim * (config.block_size**2))
